...
If you pass it an ELF file generated by `ccc`, it will wrap in, along with the CHERI-seL4 kernel, CHERI-Microkit libraries, loader, monitor, etc. to give you a bootable image and run it directly in QEMU by passing it as a -kernel image. The following is the script's content:

```sh
{{#include ../../tools/run_qemu}}
```

Thus, over these exercises, you'll usually be using these two scripts (given you either include them in your $PATH, or use relative/absolute paths when running them):
```
